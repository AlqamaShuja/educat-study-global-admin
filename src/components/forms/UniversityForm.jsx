...
const UniversityForm = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      percentageAgreed: university?.percentageAgreed || "",
    },
  });

  // Default percentage values based on MOU status
  const getDefaultPercentage = (mouStatus) => {
    switch (mouStatus) {
      case "none":
        return "10";
      case "direct":
  ...
  // Handle MOU status change and auto-set percentage
  const handleMouStatusChange = (option) => {
    const newMouStatus = option.value;
    setValue("mouStatus", newMouStatus);

    // Only auto-set percentage if current field is empty or matches a default value
    const currentPercentage = watch("percentageAgreed");
    const isDefaultValue = ["10", "5", ""].includes(currentPercentage);

    if (!currentPercentage || isDefaultValue) {
      const defaultPercentage = getDefaultPercentage(newMouStatus);
      setValue("percentageAgreed", defaultPercentage);
    }
  };
  ...
  const onFormSubmit = (data) => {
    const formData = {
      ...
      percentageAgreed: data.percentageAgreed,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Selecting MOU status will suggest default percentage values
            </p>
            <input
              type="hidden"
              {...register("mouStatus", {
          ...
          <div>
            <Input
              label="Percentage Agreed"
              type="number"
              min="0"
              max="100"
              step="0.1"
              {...register("percentageAgreed", {
                required: "Percentage agreed is required",
                min: {
                  value: 0,
              ...
              error={errors.percentageAgreed?.message}
              placeholder="Enter percentage (e.g., 10)"
              required
            />
            <p className="text-xs text-gray-500 mt-1">
              Default: No MOU (10%), Direct Partnership (10%), Third Party (5%)
            </p>
          </div>
        </div>
      </div>
