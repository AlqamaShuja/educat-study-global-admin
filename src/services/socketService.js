...
let socket = null;
const newMessageCallbacks = []; // Array to store multiple callbacks
const messageReadCallbacks = []; // Array to store multiple callbacks
...
const socketService = {
  ...
  connect(userId) {
    ...
    socket.on("messageRead", (data) => {
      ...
      messageReadCallbacks.forEach((callback) => {
        if (typeof callback === "function") {
          callback(data);
        }
      });
  ...
  onNewMessage(callback) {
    if (typeof callback === "function") {
      ...
      // Remove all callbacks if null is passed (cleanup)
